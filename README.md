...
購入者向けデモの流れは **販売開始 → 待機列 → 仮確保（TTL あり）→ 確定 → 残席のリアルタイム更新（Reverb）** です。仮確保を本人が取り消すか期限切れになると、席は待機列の先頭へ FIFO で戻ります。オークション API / 画面は残していますが、今のプロダクトデモの主軸ではありません。
...
| 主催者の在庫 | <http://localhost:3000/admin/performances> |
| ログイン（一般ユーザー） | `test_user@example.com` / `test1111` |
| 別ブラウザ用 | `demo2@example.com` / `test1111`（`demo3@example.com` も可） |
| 主催者（会場スタッフ） | `admin@example.com` / `password` |

デモ公演 **MIDNIGHT CIRCUIT VOL.12** は席数 8 のクラブナイトです。ログインして「待機列に並ぶ」と、空席があれば **仮確保** が 1 つ入り、残席が減ります。仮確保の期限は **3 分**（`TICKET_HOLD_TTL_SECONDS=180`。変更可）です。期限内に「この枠を確定する」と TTL の対象外になり、購入者画面からはキャンセルできません（決済・返金のないデモです）。確定前なら確認ダイアログ付きで取り消せます。取り消すか期限切れになると、席は待機列の先頭へ FIFO で渡り、本人には「取り消した / 期限切れ」と表示されます（抜けたように見えません）。同じアカウントで何度並んでも枠は増えません。別ブラウザ（別ユーザー）で同じ公演を開くと、残席が Reverb 経由でライブに動くのを確認できます。主催者は `/admin/login` から公演の仮確保・確定・解放履歴を見られます。決済・座席図・会場選択はありません。

TTL は API（join / status / confirm / cancel / 主催者画面）でも遅延処理します。`php artisan schedule:work` や毎分の `tickets:expire-holds` がなくてもデモは進みます。
